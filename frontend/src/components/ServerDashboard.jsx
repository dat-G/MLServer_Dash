...
export default function ServerDashboard({ serverUrl, serverName }) {
  ...
  const fetchDockerInfo = async () => {
    try {
      const response = await fetch(`${serverUrl}/api/docker`)
      const data = await response.json()
        ...
        <div className="flex flex-col md:flex-row md:justify-between gap-4">
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className={`flex items-center gap-1.5 text-sm ${wsConnected ? 'text-neon-green' : 'text-gray-500'}`}>
                <span className={`w-2 h-2 rounded-full ${wsConnected ? 'bg-neon-green animate-pulse' : 'bg-gray-500'}`} />
                {wsConnected ? '已连接' : '未连接'}
              </span>
            </div>
